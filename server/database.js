// server/database.js — Pesona Kahuripan CMS Database (Supabase Migrated)
...
if (supabaseUrl && supabaseKey) {
  ...
  console.warn('⚠️ WARNING: SUPABASE_URL and SUPABASE_KEY are not defined in environment variables. DB operations will fail!');
}
...
async function initDatabase() {
  if (!supabase) {
    console.error('Supabase client not initialized, skipping database seeding');
    return;
  }

  try {
    // Check if we already have hero data (which means it's seeded)
    ...
    const defaults = {
      ...
      settings: {
        ...
        founded_year: '2013',
        instagram: 'https://www.instagram.com/pesonakahuripan',
        facebook: 'https://www.facebook.com/pesonakahuripan',
      ...
      news: [
        ...
        },
        {
          ...
          excerpt: "Setelah peluncuran sukses, PT Bank Tabungan Negara Tbk (BTN) menyelenggarakan akad massal perdana untuk 200 unit KPR subsidi and non-subsidi di cluster terbaru Pesona Kahuripan 11. Proses administrasi akad massal yang efisien ini membantu para pembeli menerima serah terima kunci rumah impian secara instan.",
          order: 4,
          created_at: new Date().toISOString()
        },
        {
